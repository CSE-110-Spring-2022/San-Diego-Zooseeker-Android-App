...
public class ExhibitRoute {
    ...
    public String getInstruction(int current,Coord location) {
        ...
        Pair<Double,String> distance= this.findClosest(current_location,target_location,location);
        return findPathBetween(current_location, target_location, distance.first);
    }
    ...
    private String findPathBetween(String current_location, String target_location, double distance_from_current) {
        int s = 0;
        ...
        for (int i = s; i < t; i++) {
            ...
            if (i == s) {
                ret += String.format("Walk %s meters along %s from %s to %s.\n\n",
                        distance_from_current + "ft", edges.get(i).street, vertices.get(i).name, vertices.get(i + 1).name);
            } else {
                ret += String.format("Walk %s meters along %s from %s to %s.\n\n",
